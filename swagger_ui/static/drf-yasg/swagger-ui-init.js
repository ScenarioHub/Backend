...
var swaggerUiConfig = {
    ...
    requestInterceptor: function (request) {
        ...
        // ---- Bearer 자동 접두어 추가 시작 ----
        // swagger authorize에 토큰만 입력해도 Authorization: Bearer <token>로 전송
        var authKey = null;
        if ("Authorization" in headers) authKey = "Authorization";
        else if ("authorization" in headers) authKey = "authorization";

        if (authKey) {
            var raw = String(headers[authKey] || "").trim();

            // 이미 "Bearer xxx" 같은 스킴이 있으면 그대로 둠
            // (bearer/basic/token 등 '단어+공백' 패턴이면 스킴이 있다고 판단)
            if (raw && !/^\w+\s+/i.test(raw)) {
